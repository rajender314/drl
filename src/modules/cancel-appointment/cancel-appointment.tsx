...
import { Data } from '../prescription-detail/prescription-detail-component';
...
export default function CancelAppointment() {
    ...
    const getAppointmentDetails = async () => {
        await appointmentDetails(appointmentId).then((res: any) => {
            ...
            getCancellationReasonsApi('doctor')

        });
    }
    const getCancellationReasonsApi = async (type: any) => {
        await getCancellationReasons(type).then((res: any) => {
            if (res.status === 200) {
                setCancelReason(res.data.cancellation_reasons)
            }
        }).finally(() => {
            setSpinner(false)
        });
    ...
    const cancelAppointment = async () => {
        ...
        // if (reason.toLowerCase() == 'changed my mind') {
        //     history.goBack()
        //     return
        // }
        ...
        if (appointmentId) {
            ...
            await cancelAppointmentApi(params).then((res) => {
                if (res.status && res.status == 200) {
                    setValid(true)
                } else {
                    setValid(false)
                    setApiErrorMsg(res.data.message || res.data.error)
                }
            ...
            setSpinner(true)
            await cancelOrderApi(params).then((res: any) => {
                //console.log(res)
                if (res.status && res.status == 200) {
                    console.log('valid')
                    setValid(true);
    ...
    if (appointmentId && appointmentData &&
        (appointmentData.status === 'CONFIRMED ' || appointmentData.status === 'ACCEPTED' ||
            appointmentData.status === 'PENDING') && !appointmentData.isWebLinkConfigured) {

    } else if (appointmentId) {
        //console.log(appointmentId, orderId)
        navigate(`/files/appointments/${appointmentId}`)
        ...
        <Container>
            ...
            <CancelCard>
                <CancelContainer>
                    ...
                    {RadioInputs()}
                    {(appointmentId && selReason.toLowerCase() === "my reason is not listed") || (orderId && selReason.toLowerCase() === "my reason is not listed") ? (
